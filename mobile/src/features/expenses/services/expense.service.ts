...
export const ExpensesService = {
  getAllExpenses: async (): Promise<ExpenseType[]> => {
    const expenses = await DATABASE.db.getAllAsync("SELECT * FROM expenses ORDER BY date DESC");
  ...
  getExpenseById: async (id: number): Promise<ExpenseType | null> => {
    const exp: any = await DATABASE.db.getFirstAsync("SELECT * FROM expenses WHERE id = ?", [id]);
    if (!exp) return null;
    return {
      ...exp,
      date: new Date(exp.date),
  ...
  createExpense: async (expense: Omit<ExpenseType, 'id'>): Promise<ExpenseType> => {
  ...
  updateExpense: async (id: number, expense: Partial<Omit<ExpenseType, 'id'>>): Promise<void> => {
    const fields: string[] = [];
    const values: any[] = [];
  ...
  getTodayExpenses: async (): Promise<ExpenseType[]> => {
    ...
    const expenses = await DATABASE.db.getAllAsync("SELECT * FROM expenses WHERE date = ?", [todayStr]);
    return expenses.map((exp: any) => ({
      ...exp,
      date: new Date(exp.date),
    })) as ExpenseType[];
  ...
  getExpensesByDateRange: async (startDate: string, endDate: string): Promise<ExpenseType[]> => {
  ...
  getTotalByCategory: async (): Promise<{ category: string; total: number }[]> => {
